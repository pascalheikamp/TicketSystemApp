...
@section('content')
    <form method="post" action="{{ route('update', $ticket->id) }}" class=" flex flex-col mt-20 mx-auto w-full max-w-lg">
        @csrf
        @method('PUT')
        <h1 class=" text-center text-xl font-semibold">Edit ticket</h1>
        <div class=" pt-10 flex flex-wrap -mx-8 mb-6">
            <div class="w-full md:w-1/2 px-3 mb-6 md:mb-0">
                ...
                <input
                    ...
                    value="{{$ticket->title}}" name="title" id="title" type="text"
                   >
                @error('title')
            ...
            <div class="w-full md:w-1/2 px-3">
                ...
                <textarea
                    ...
                    id="w3review" name="description" rows="4" cols="50" value="test"> {{$ticket->description}}
</textarea>
                @error('description')
                <span class="flex items-center font-medium tracking-wide text-red-500 text-xs mt-1 ml-1" role="alert">
                                        <strong>{{ $message }}</strong>
